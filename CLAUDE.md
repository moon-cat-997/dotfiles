...
So the directory a repo lands in is what makes `includeIf` pick the matching identity — use `hat clone` rather than `git clone` directly to keep identity and location consistent. Run `git config user.email` in any repo to confirm the identity it resolves to.
